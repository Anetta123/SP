...
double half__interval(double a, double b, double fa, double fb) {
    double root (0.);
    total__iterations = 0.;
    root = (fa < 0. && fb > 0. ? __MAB__try(a, b) 
    : fa > 0. && fb < 0. ? __MAB__try(b, a)
    : (b + 1.));
    newline();
    return root;
}
double __MAB__try(double neg__point, double pos__point) {
    ...
    : (test__value = f(midpoint), display("+"), total__iterations = total__iterations + 1., test__value > 0. ? __MAB__try(neg__point, midpoint) 
